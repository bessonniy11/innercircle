...
3.  **Проверка отправки и получения сообщений (Требует доработки real-time функционала):**
    *   На первом устройстве введите сообщение в поле ввода и нажмите кнопку "Отправить".
    *   Убедитесь, что ваше сообщение появилось в чате **справа**. В данный момент оно будет отображаться только локально.
    *   На втором устройстве **сообщение НЕ появится автоматически в реальном времени**. Для его отображения необходимо перезайти в чат или обновить экран.
    *   Отправка сообщений будет успешно сохраняться в базе данных, но real-time обновление пока не работает.
