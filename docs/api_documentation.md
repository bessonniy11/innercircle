...
- ⚠️ **TURN сервер:** планируется в будущем для сложных сетей
...
**Следующие шаги:**
- 🎯 **TURN сервер** - для 100% покрытия сетей
...
## Модуль звонков (WebRTC)

### Жизненный цикл звонка и обмен сигналами

#### Устраненная проблема: Гонка состояний `callId: null`

В ходе отладки (06.09.2025) была выявлена и устранена гонка состояний на стороне frontend-клиента.

**Проблема:** Клиент начинал сбор ICE-кандидатов и их отправку на сервер немедленно после инициации звонка, не дожидаясь, пока сервер создаст сессию звонка и вернет ее уникальный идентификатор (`callId`). Это приводило к тому, что бэкенд получал кандидаты с `callId: null` и отбрасывал их, что вызывало одностороннюю слышимость.

**Решение:** Логика на стороне клиента (`webrtc_service.dart`) была изменена. Теперь клиент придерживает (буферизирует) все сгенерированные ICE-кандидаты до тех пор, пока не получит от сервера событие `call_initiated` с валидным `callId`. После получения `callId` клиент отправляет все накопленные кандидаты.

**Вывод:** Данное изменение значительно повысило надежность установки WebRTC-соединения и является важной частью сигнального протокола. API бэкенда при этом не изменялся, но его корректная работа теперь гарантируется исправленной логикой клиента.
